...
function prepareAchievement(a: A, index: number): Prepared {
  const visualReferenceInstruction = a.visualReferenceUrl
    ? `REFERÊNCIA EXOPHASE — INSPIRAÇÃO, NÃO REPRODUÇÃO: use esta imagem individual do Exophase como referência visual e contextual para compreender o conceito da conquista. Preserve o significado da conquista, a atmosfera, os elementos conceituais importantes e a linguagem geral do jogo, mas crie uma interpretação visual original para o Rumo à Conquista. Não reproduza fielmente a imagem, não copie personagens, ilustrações, ícones, logos ou outros elementos protegidos e não tente recriar a referência pixel a pixel. Se a referência apresentar um personagem ou elemento protegido, substitua-o por uma representação original que transmita a mesma ideia sem reproduzi-lo diretamente. A distância criativa deve ser suficiente para manter a arte original e dentro das diretrizes do ChatGPT. Referência visual individual para inspiração: ${a.visualReferenceUrl}`
  ...
  return {
    ...
    visualConcept: a.description
      ? `Crie UMA imagem individual de conquista para "${a.name}". Gere somente esta conquista nesta imagem, sem outras conquistas, sem colagem e sem painel. A saída deve ser PNG 1:1 com fundo transparente fora da moldura e sem texto, usando o conceito específico de "${a.name}" e os elementos da descrição. Quando houver referência Exophase, siga a regra de INSPIRAÇÃO, NÃO REPRODUÇÃO acima: use a referência para entender o conceito, a atmosfera e a identidade do jogo, mas produza uma interpretação visual original. Não reproduza fielmente personagens, ilustrações, ícones, logos ou outros elementos protegidos. Não aplique uma fórmula visual obrigatória a todos os jogos; preserve a vibe do jogo sem copiar a arte de referência. ${visualReferenceInstruction}`
                  ...
                  {Array.from(
                    ...
                    (_, batchIndex) => {
                      ...
                      const lines = [
                        ...
                        "Composição de CADA arquivo: quando houver referência Exophase, use a composição e o enquadramento apenas como inspiração, fazendo uma interpretação original e segura para o Rumo à Conquista. Não reproduza fielmente a arte de referência. Quando não houver referência, o símbolo principal deve ocupar aproximadamente 60–75% da imagem.",
                        "ESTILO E SEGURANÇA: NÃO aplique uma fórmula visual fixa de fundo vermelho, metal, dourado, moldura ou 3D a todas as conquistas. Quando houver referência Exophase, use-a como inspiração para a vibe, atmosfera, conceito e linguagem do jogo, mas crie uma arte original. Não copie nem reproduza fielmente personagens, ilustrações, ícones, logos ou outros elementos protegidos. Se necessário, substitua elementos protegidos por equivalentes originais que transmitam a mesma ideia. A identidade do jogo e da conquista tem prioridade sobre uma estética genérica, sempre respeitando as diretrizes do ChatGPT.",
                        ...
                        "REFERÊNCIA EXOPHASE — INSPIRAÇÃO, NÃO REPRODUÇÃO: quando houver uma referência visual individual do Exophase, use-a para compreender o conceito da conquista, sua atmosfera, composição geral, elementos conceituais e linguagem visual do jogo. Crie uma interpretação original para o Rumo à Conquista. Não reproduza fielmente a imagem, não copie personagens, ilustrações, ícones, logos ou outros elementos protegidos e não tente recriar a referência pixel a pixel. Se a referência apresentar um personagem ou elemento protegido, substitua-o por uma representação original que transmita a mesma ideia. A prioridade é criar uma arte original, reconhecível como a conquista e coerente com a vibe do jogo, dentro das diretrizes do ChatGPT.",
