...
function prepareAchievement(a: A, index: number): Prepared {
  const visualReferenceInstruction = a.visualReferenceUrl
    ? `REFERÊNCIA EXOPHASE — REGRA 80/20: use esta imagem individual do Exophase como a referência principal da conquista. Preserve aproximadamente 80% da identidade visual essencial da referência: o símbolo ou objeto principal, personagem, silhueta, conceito, elementos característicos e cores que fazem esta conquista ser imediatamente reconhecível. Os outros aproximadamente 20% são a adaptação do Rumo à Conquista: melhore o acabamento, limpeza, profundidade, iluminação e tratamento visual sem transformar a conquista em outro conceito. A arte final deve parecer uma versão aprimorada da referência, e não uma criação genérica ou uma nova interpretação distante. Não copie, recorte, filtre, redesenhe pixel a pixel ou simplesmente reproduza a imagem original. NÃO substitua a referência por outra imagem de outra base. Referência visual individual: ${a.visualReferenceUrl}`
                  ...
                  {Array.from(
                    ...
                    (_, batchIndex) => {
                      ...
                      const lines = [
                        ...
                        "REGRA 80/20 — REFERÊNCIA EXOPHASE: quando uma conquista tiver uma referência visual individual do Exophase, ela é a referência principal. Preserve aproximadamente 80% da identidade visual essencial da referência — símbolo/objeto principal, personagem, silhueta, conceito, elementos característicos e cores predominantes — para que a conquista continue imediatamente reconhecível como a mesma conquista. Os aproximadamente 20% restantes são a adaptação do Rumo à Conquista: melhore acabamento, limpeza, profundidade, iluminação, detalhes e tratamento visual. A arte final deve ser uma versão aprimorada da referência, não uma criação genérica e nem uma interpretação que transforme o conceito em outra coisa.",
                        "NÃO copie, recorte, filtre, redesenhe pixel a pixel ou simplesmente reproduza a imagem do Exophase. Não substitua a referência por imagens de Steam, Xbox ou outras bases. Preserve a identidade da referência em vez de aplicar uma fórmula visual obrigatória igual para todos os jogos.",
